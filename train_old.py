...
def main(args):
    ...
    ignore_substrings = dataset_cfg["ignore_substrings"]

    def exclude_if_contains(path: Path) -> bool:
        return not any(bad in str(path) for bad in ignore_substrings)
    ...
    val_dataset_length = val_dataset_cfg["length"]
    ignore_val_substrings = val_dataset_cfg["ignore_substrings"]

    def exclude_if_contains(path: Path) -> bool:
        return not any(bad in str(path) for bad in ignore_val_substrings)

    val_dataset = CSVDataset(
        tracker_csv_dir_name=val_dataset_cfg["tracker_csv_dir_name"],
        embedding_csv_dir_name=val_dataset_cfg["embedding_csv_dir_name"],
        gt_csv_dir_name=val_dataset_cfg["gt_csv_dir_name"],
        run=val_dataset_cfg["run"],
        filter_fn=exclude_if_contains,
        candidate_graph_dir_name=val_dataset_cfg["candidate_graph_dir_name"],
        ...
        if (iteration + 1) % log_loss_every == 0:
            ...
            with torch.no_grad():
                for val_x in val_loader:
                    ...
                    val_x = val_x.view(
                        val_x.shape[0] * val_x.shape[1], val_x.shape[-1], 1
                    )
